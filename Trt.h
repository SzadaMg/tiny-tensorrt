...
// Version compatibility macros for TRT 8.0-10.x
...
template <typename T> struct TrtDestroyer {
  void operator()(T *t) {
    ...
    t->destroy();
#else
    // TRT 8.0+ (including TRT 10) uses delete instead of destroy()
    delete t;
#endif
  }
};
...
public:
  ...
  /**
   * Setting a int8 calibrator.To run INT8 calibration for a network with
   * dynamic shapes, calibration optimization profile must be set. Calibration
   * is performed using kOPT values of the profile. Calibration input data size
   * must match this profile.
   * @calibratorData: use for int8 mode, calabrator data is a batch of sample
   * input, for classification task you need around 500 sample input. and this
   *                  is for int8 mode
   * @calibratorType: there are 3 calibrator types now.
   *                  "EntropyCalibratorV2" : This is the recommended calibrator
   * and is required for DLA. Calibration happens before Layer fusion by
   * default. This is recommended for CNN based networks. "MinMaxCalibrator" :
   * This is the preferred calibrator for NLP tasks for all backends.
   *                  Calibration happens before Layer fusion by default. This
   * is recommended for BERT like networks. "EntropyCalibrator" : This is the
   * legacy entropy calibrator.This is less complicated than a legacy calibrator
   * and produces better results. Calibration happens after Layer fusion by
   * default. See kCALIBRATION_BEFORE_FUSION for enabling calibration before
   * fusion.
   */
  ...
  /**
   * Set the maximum GPU temporary memory which the engine can use at execution
   * time.
   * @deprecated This method was deprecated in TRT 8.4 and removed in TRT 10.0
   *            Use IBuilderConfig::setMemoryPoolLimit() instead for TRT 8.4+
   */
   ...
   * @level Severity::kINTERNAL_ERROR = 0, Severity::kERROR = 1,
   * Severity::kWARNING = 2, Severity::kINFO = 3, Severity::kVERBOSE = 4,
   ...
   * @engineFile: path to saved engien file will be save, if it's empty them
   * will not save engine file
   */
  void BuildEngine(const std::string &onnxModel, const std::string &engineFile);

  /**
   * Deserialize an engine from engineFile
   * Note: If your model has dynamic shapes, you must call
   * AddDynamicShapeProfile before DesirializeEngine as you did when building
   * engien from onnx.
   * @engineFile: can be create by BuildEngine, or save with trtexec or
   * tiny-exec
   * @dlaCore: dla core to use, you can build engine on dla core 0 and
   * deserialize the engine to core 1. Only available on jetson platform has DLA
   * support. return false if deserialization failed.
   */
  bool DeserializeEngine(const std::string &engineFile, int dlaCore = -1);

  /**
   * Do inference on engine context, make sure you already copy your data to
   * device memory, return true if success
   */
  bool Forward();
  ...
  /**
   * Set input dimentiosn for an inference, call this before forward with
   * dynamic shape mode.
   */
  ...
  /**
   * Get binding data pointer in device. for example if you want to do some post
   * processing on inference output but want to process them in gpu directly for
   * efficiency, you can use this function to avoid extra data IO, or you can
   * copy inputs from device to binding prt directly so that you don't need to
   * call CopyFromHostToDevice. Hence good for performance.
   * @return: pointer point to device memory.
   */
  void *GetBindingPtr(int bindIndex) const;

  /**
   * Get binding data size in byte, so maybe you need to divide it by sizeof(T)
