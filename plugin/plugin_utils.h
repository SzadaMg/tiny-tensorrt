...
#define cutelog(...)                                                           \
  {                                                                            \
    ...
    std::cout << " (๑¯◡¯๑) noexcept CUSTOM PLUGIN TRACE----> call " << "["     \
 ...
 * @描述:
 * 这是实现自定义插件的时候非常常用的一些函数,我将它们抽离出来,你可以直接调用即可
 */

// this is for debug, and you can find a lot assert in plugin implementation,
// it will reduce the time you spend on debug
// 为了方便debug, 你也可以在插件的实现里面看到我大量使用了assert,
// 这个可以大大减少花在 debug上的时间
#define ASSERT(assertion)                                                      \
  {                                                                            \
    if (!(assertion)) {                                                        \
      std::cerr << "#assertion fail " << __FILE__ << " line " << __LINE__      \
                << std::endl;                                                  \
      abort();                                                                 \
    }                                                                          \
  }
...
void convertAndCopyToDeivce(void *&deviceWeights,
