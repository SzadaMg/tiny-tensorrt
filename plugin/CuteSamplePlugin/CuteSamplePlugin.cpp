...
#ifndef CUTEDEBUG
#define CUTEDEBUG                                                              \
  0 // set debug mode, if you want to see the api call, set it to 1
#endif
...
CuteSamplePlugin::CuteSamplePlugin() { cutelog("wow I run to here now"); }
...
Dims CuteSamplePlugin::getOutputDimensions(int index, const Dims *inputs,
                                           int nbInputDims) IS_NOEXCEPT {
  cutelog("wow I run to here now");
...
DataType
CuteSamplePlugin::getOutputDataType(int index,
                                    const nvinfer1::DataType *inputTypes,
                                    int nbInputs) const IS_NOEXCEPT {
  cutelog("wow I run to here now");
  return DataType::kFLOAT;
}

// Enqueue signature changed in TRT 8.0+ (including TRT 10.x)
#if NV_TENSORRT_MAJOR < 8
int CuteSamplePlugin::enqueue(int batchSize, const void *const *inputs,
                              void **outputs, void *workspace,
                              cudaStream_t stream)
#else
int CuteSamplePlugin::enqueue(int batchSize, void const *const *inputs,
...
bool CuteSamplePlugin::isOutputBroadcastAcrossBatch(
...
void CuteSamplePlugin::configurePlugin(
    ...
    PluginFormat floatFormat, int maxBatchSize) IS_NOEXCEPT {
  cutelog("wow I run to here now");
}

bool CuteSamplePlugin::supportsFormat(DataType type,
                                      PluginFormat format) const IS_NOEXCEPT {
  cutelog("wow I run to here now");
  return true;
}
...
CuteSamplePluginCreator::getFieldNames() IS_NOEXCEPT {
  cutelog("wow I run to here now");
  return &mFC;
}

IPluginV2Ext *CuteSamplePluginCreator::createPlugin(
...
IPluginV2Ext *CuteSamplePluginCreator::deserializePlugin(
