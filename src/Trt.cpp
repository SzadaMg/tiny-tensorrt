...
bool setTensorDynamicRange(const nvinfer1::INetworkDefinition &network,
                           float inRange, float outRange) {
...
void TrtLogger::log(Severity severity, const char *msg) TRT_NOEXCEPT {
...
void TrtLogger::setLogSeverity(Severity severity) { mSeverity = severity; }
...
Trt::Trt() {
  ...
  // setMaxWorkspaceSize is deprecated in TRT 8.4+ and removed in TRT 10.0
  // Use setMemoryPoolLimit() for TRT 8.4+ instead
  mConfig->setMaxWorkspaceSize(1 << 30); // 1GB
#endif
  mProfile = mBuilder->createOptimizationProfile();
  assert(mProfile != nullptr &&
         "create trt builder optimazation profile failed");
}
...
void Trt::EnableFP16() {
  assert(mBuilder != nullptr && mConfig != nullptr &&
         "Please set config before build engine");
...
void Trt::SetInt8Calibrator(const std::string &calibratorType,
...
// SetWorkpaceSize is deprecated in TRT 8.4 and removed in TRT 10.0
// Use IBuilderConfig::setMemoryPoolLimit() for TRT 8.4+ instead
#if NV_TENSORRT_MAJOR < 10 &&                                                  \
    !(NV_TENSORRT_MAJOR >= 8 && NV_TENSORRT_MINOR >= 4)
...
#endif // NV_TENSORRT_MAJOR < 10 && !(NV_TENSORRT_MAJOR >= 8 &&
       // NV_TENSORRT_MINOR >= 4)
...
void Trt::AddDynamicShapeProfile(const std::string &inputName,
  ...
  mProfile->setDimensions(inputName.c_str(), nvinfer1::OptProfileSelector::kMIN,
                          minDim);
  mProfile->setDimensions(inputName.c_str(), nvinfer1::OptProfileSelector::kOPT,
                          optDim);
  mProfile->setDimensions(inputName.c_str(), nvinfer1::OptProfileSelector::kMAX,
                          maxDim);
...
void Trt::BuildEngine(const std::string &onnxModel,
  ...
  bool parse_success = parser->parseFromFile(
      onnxModel.c_str(), static_cast<int>(Severity::kWARNING));
  ...
  if (mConfig->getFlag(nvinfer1::BuilderFlag::kINT8) &&
      mConfig->getInt8Calibrator() == nullptr) {
  ...
  // Engine building API differs between TRT 7 and TRT 8+
  // TRT 8.0+ (including TRT 10.x) uses buildSerializedNetwork
#if NV_TENSORRT_MAJOR < 8
...
#if NV_TENSORRT_MAJOR >= 10
bool Trt::Forward() {
  // TensorRT 10.x uses enqueueV3 with tensor addresses
  for (size_t i = 0; i < mBindingName.size(); i++) {
    if (!mContext->setTensorAddress(mBindingName[i].c_str(), mBinding[i])) {
      return false;
    }
  }
  return mContext->enqueueV3(0);
}
...
#else
bool Trt::Forward() { return mContext->executeV2(&mBinding[0]); }
...
#if NV_TENSORRT_MAJOR >= 10
  // TensorRT 10.x uses tensor names instead of binding indices
  mContext->setInputShape(mBindingName[bindIndex].c_str(), dims);
#else
...
#ifdef DEBUG
  spdlog::info("input size: {}, binding size: {}", input.size() * sizeof(float),
               mBindingSize[bindIndex]);
#endif
...
void Trt::CopyFromDeviceToHost(std::vector<float> &output, int bindIndex,
                               const cudaStream_t &stream) {
#ifdef DEBUG
  spdlog::info("output size: {}, binding size: {}",
               output.size() * sizeof(float), mBindingSize[bindIndex]);
#endif
...
void *Trt::GetBindingPtr(int bindIndex) const { return mBinding[bindIndex]; }
...
int Trt::GetNbInputBindings() const { return mNbInputBindings; }

int Trt::GetNbOutputBindings() const { return mNbOutputBindings; }

void Trt::CreateDeviceBuffer() {
  spdlog::info("malloc device memory");
#if NV_TENSORRT_MAJOR >= 10
  // TensorRT 10.x uses tensor-based API
  int nbBindings = mEngine->getNbIOTensors();
  spdlog::info("nbIOTensors: {}", nbBindings);
  ...
  for (int i = 0; i < nbBindings; i++) {
    ...
    if (mIsDynamicShape) {
      if (ioMode == nvinfer1::TensorIOMode::kINPUT) {
        dims =
            mProfile->getDimensions(name, nvinfer1::OptProfileSelector::kMAX);
        mContext->setInputShape(name, dims);
      } else {
        assert(mContext->allInputShapesSpecified());
        dims = mContext->getTensorShape(name);
      }
    } else {
      dims = mEngine->getTensorShape(name);
    }
    ...
    spdlog::info("binding bindIndex: {}, name: {}, size in byte: {}", i, name,
                 totalSize);
...
#else
  // TensorRT 8.x-9.x uses binding-based API
